...
function generaImmagine(i, j) {
    ...
    do {
        ...
        (grid[i - 1] && grid[i - 1][j] === imagePath && grid[i - 2] && grid[i - 2][j] === imagePath && grid[i - 3] && grid[i - 3][j] === imagePath) ||
        (grid[i] && grid[i][j - 1] === imagePath && grid[i][j - 2] === imagePath && grid[i][j - 3] === imagePath)
    );
    return imagePath;
}
...
function rimuoviCelle(i, j, direzione) {
    let celleDaRimuovere = direzione === 'orizzontale' ? [[i, j], [i, j + 1], [i, j + 2], [i, j + 3]] : [[i, j], [i + 1, j], [i + 2, j], [i + 3, j]];
